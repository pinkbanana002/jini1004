...
_PROMPT = """\
이 이미지는 쿠팡 상세페이지용으로 쓸 상품 이미지인데, 중국어(간체/번체) 텍스트가
배너나 라벨 형태로 이미지 위에 오버레이 되어 있다. 이미지 안에 있는 모든 중국어
텍스트 블록을 찾아서 아래 JSON 형식으로만 응답해라. 다른 설명, 코드펜스, 마크다운
없이 순수 JSON 배열만 출력해라.

[
  {
    "text_original": "원본 중국어 텍스트",
    "text_ko": "자연스러운 한국어 번역 (짧고 상세페이지 배너에 어울리게)",
    "box_1000": [ymin, xmin, ymax, xmax]
  },
  ...
]

box_1000 좌표는 이미지 가로/세로를 각각 0~1000 스케일로 정규화한 값이다.
텍스트가 없으면 빈 배열 [] 을 반환해라.
"""
...
def translate_text_regions(
    ...
    for r in regions:
        ...
        if not box or len(box) != 4 or not text_ko:
            continue
